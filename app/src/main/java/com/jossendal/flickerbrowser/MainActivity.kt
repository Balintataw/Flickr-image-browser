...
class MainActivity : BaseActivity(), GetRawData.OnDownloadComplete,
    ...
    override fun onItemClick(view: View, position: Int) {
        Toast.makeText(this, "Tap on position: $position", Toast.LENGTH_SHORT).show()
    }
...
//            getFlickrJsonData.execute("cat")
            getFlickrJsonData.execute(data)
        } else {
            Log.d(TAG, "onDownloadCompleteFailed with status $status. Error message is: $data")
        }
    }
    ...
    override fun onError(exception: Exception) {
        Log.d(TAG, "onErrorCalled: $exception")
    }
